...
public class UserAlbumWallApi {
    ...
    public ResponseEntity getMineAlbumWall(@AuthenticationPrincipal UserLogin authUser) {
        List<PhotoWall> albumWalls = userPhotoWallService.findAllByUid(authUser.getUid());
    ...
    public ResponseEntity getAlbumWall(@PathVariable("uid") Long uid) {
        List<PhotoWall> albumWalls = userPhotoWallService.findAllByUid(uid);
        return ResponseEntity.ok(albumWalls);
    }
...
//    @PreAuthorize("hasRole('USER')")
    public ResponseEntity add(@AuthenticationPrincipal UserLogin authUser,
                              MultipartFile image) {
//        MultipartFile[] images = albumWallDTO.getImages();
        MultipartFile[] images = {image};
        if (images == null)
            return ResponseEntity.badRequest().body("请传入至少一张图片");
        try {
